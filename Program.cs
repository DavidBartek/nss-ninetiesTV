...
{
    return shows.OrderByDescending(s => s.Name.Count()).First();
}
...
{
    string allNames = string.Join(", ", shows.Select(s => s.Name));
    return allNames;
}
...
{
    string allNamesButLastTwo = string.Join(", ", shows.SkipLast(2).Select(s => s.Name));
    string lastTwoNames = string.Join(", and ", shows.TakeLast(2).Select(s => s.Name));
    return allNamesButLastTwo + ", " + lastTwoNames;
}
